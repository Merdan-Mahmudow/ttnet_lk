...
import { Circle, Float, Icon, Span, Portal, CloseButton, Dialog, IconButton, For, Card, Heading, Stack } from "@chakra-ui/react"
...
export function Header({ isMobile, title, toggleAside }: AsideProps & { toggleAside: () => void }) {
    ...
    const notifications: TNotifyCard[] = [
        { title: "Новый заказ", description: "Поступил новый заказ №348 от клиента Иван П." },
        { title: "Заказ готов", description: "Заказ №346 готов к выдаче в зале." },
        { title: "Клиент отменил заказ", description: "Клиент отменил заказ №345. Проверьте детали в системе." },
        { title: "Обновление меню", description: "Добавлена новая позиция: Пицца «Барбекю XXL»." },
        { title: "Окончание смены", description: "Смена сборщика №2 завершится через 30 минут." },
        { title: "Нет ингредиентов", description: "Закончился сыр моцарелла. Обновите склад." }
    ];
                    ...
                    <Portal>
                        ...
                        <Dialog.Positioner>
                            <Dialog.Content bg={"white"} color={"black"} rounded={"xl"}>
                                <Dialog.Header bg={color.ACCENT} color={"white"} borderTopRadius={"xl"}>
                                ...
                                <Dialog.Body>
                                    <Stack>
                                        <For each={notifications}>
                                            {(item, index) => (
                                                <Card.Root size={"sm"} key={index} bg={color.GRAY_50} border={"none"}>
                                                    <Card.Header>
