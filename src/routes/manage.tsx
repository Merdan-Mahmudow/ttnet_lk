...
function RouteComponent() {
  ...
  return (
    ...
    >
      <HStack>
        <Circle bg={isNetworkOpen ? color.ACCENT : color.ERROR} p={".5em"}>
          <WifiIcon width="25" height="25" fill="white" />
        </Circle>
        <Text fontWeight={"600"} fontSize={"20px"}>
          Интернет {isNetworkOpen ? "включён" : "выключен"}
        </Text>
        <Switch.Root
      ...
      <HStack marginBottom={"10px"}>
        <Text fontWeight={"600"} fontSize={"20px"}>
          {isSheduleOpen ? "Выключить" : "Включить"} расписание
        </Text>
        <Switch.Root
          size={"lg"}
          colorPalette={"green"}
      ...
      {isSheduleOpen && (
        <Box minW={["90vw", "80vw", "90%"]}>
          ...
          <Box bg={color.GRAY_50} rounded={"xl"}>
            <HStack
              padding={"10px"}
              align={"center"}
              justifyContent={"space-around"}
            >
              <Heading fontSize={["sm", "lg", "lg"]} color={color.ACCENT}>
                День недели
              </Heading>
              <HStack pos={"relative"} right={["0", "1", "3"]}>
                <Circle bg={color.ACCENT} p={".5em"}>
